...
const product_details = {
    ...
    // ],
    colors: [
        { name: "black", img_src: "/resources/products/1_black.jpg", stock_size: 25 },
        { name: "black", img_src: "/resources/products/1_black_2.jpg", stock_size: 25 },
        { name: "blue", img_src: "/resources/products/1_blue.jpg", stock_size: 25 },
        { name: "green", img_src: "/resources/products/1_green.jpg", stock_size: 10 },
        { name: "green", img_src: "/resources/products/1_green_2.webp", stock_size: 10 },
        { name: "green", img_src: "/resources/products/1_green_3.jpg", stock_size: 10 },
        { name: "pink", img_src: "/resources/products/1_pink.jpg", stock_size: 25 },
    ],
...
export default function ProductGrid() {
    ...
    const product_size = useRef();
    const max_stock_size = useRef(10);
    const selected_size_stock_size = useRef(Number.MAX_SAFE_INTEGER);
    const selected_color_stock_size = useRef(Number.MAX_SAFE_INTEGER);
    ...
    const handleSizeChange = (event, stock_size) => {
        const { value, checked } = event.target;
        if (checked) {
            product_size.current = value
            selected_size_stock_size.current = stock_size
            if (selected_size_stock_size.current < selected_color_stock_size.current) {
                max_stock_size.current = selected_size_stock_size
            }
        }
    }

    const handleColorChangeByImage = (event, image_url, color_name, stock_size) => {
        setProductImage_url(image_url)
        selected_color_stock_size.current = stock_size
        if (selected_color_stock_size.current < selected_color_stock_size.current) {
            max_stock_size.current = selected_size_stock_size.current
        }
    }
    ...
    return (<>
        <div className="mt-12 -z-50">
            <div id={productGridId} className="w-full grid gap-1.5 sm:gap-2 md:gap-3 2md:gap-2 grid-cols-2 sm:grid-cols-3 2md:grid-cols-4 lg:grid-cols-6">
                                    ...
                                    <div className="md:h-full flex-1 md:overflow-y-auto">
                                        ...
                                        <div className="text-base 2md:text-xl xl:text-2xl">
                                                ...
                                                <div className="flex items-center">
                                                    <div className="flex items-center">
                                                        {[0, 1, 2, 3, 4].map((rating) => (
                                                            <svg key={rating} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className={`w-5 h-5 lg:w-6 lg:h-6 ${rating > product_details.rating ? "text-gray-300" : "text-[#cf10a5]"}`}>
                                            ...
                                            <div className="mt-1 mb-4 font-medium">
                                                <span className="text-gray-400 text-sm 2md:text-lg xl:text-xl pr-3">
                                                    ...
                                                    <del>{product_details.price}.00</del>
                                                </span>
                                                <span className="text-base 2md:text-xl xl:text-2xl">
                                                    <span className="font-serif">৳</span>
                                                    {product_details.discount_price}.00
                                                </span>
                                            </div>
                                        </div>
                                            ...
                                            <ul className="h-2-scrollbar flex items-center h-full gap-2 sm:gap-4 text-sm overflow-x-auto">
                                                {product_details.sizes.map((size, size_index) => (
                                                    <li key={size_index} className="h-auto w-auto">
                                                        <input id={`size-input-check-${productGridId + product_index}-${size_index}`} type="radio" name={"size"} value={size.name} className="peer hidden" onChange={(e) => handleSizeChange(e, size.stock_size)} />
                                        ...
                                        <div className="h-12 md:h-16 w-full flex items-center font-semibold font-sans">
                                            ...
                                            <ul className="h-2-scrollbar flex items-center h-full gap-3 sm:gap-5 text-sm overflow-x-auto px-2.5">
                                                {product_details.colors.map((product_color, color_index) => (
                                                    <li key={color_index} className="h-5/6 aspect-square">
                                                        <input id={`color-input-check-${productGridId + product_index}-${product_color.img_src}`} type="radio" name={"grid_product_color"} value={product_color.name} className="peer hidden" />
                                                        <label htmlFor={`color-input-check-${productGridId + product_index}-${product_color.img_src}`} className={`block rounded-md h-full w-full cursor-pointer overflow-hidden peer-checked:ring-offset-2 peer-checked:ring-2 peer-checked:ring-[#FF375F] peer-checked:border-0 border border-gray-500`}>
                                                            <Image
                                                                ...
                                                                onClick={(e) => handleColorChangeByImage(e, product_color.img_src, product_color.name)}
                                                            />
                                                        </label>
                                                    </li>
                                                ))}
