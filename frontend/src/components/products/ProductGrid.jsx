...
export default function ProductGrid() {
    ...
    const handleColorChange = (event) => {
        ...
        const selected_color_image = product_details.colors.find(color => color.name === element.value);
        const nextElementSibling = element.closest('ul').parentNode.nextElementSibling
        const allLabels = nextElementSibling.querySelectorAll('ul .ring-2');
        allLabels.forEach(label => {
            label.classList.remove("ring-offset-2", "ring-2", "ring-[#FF375F]")
            label.classList.add("border")
        });
        const selected_image_label = nextElementSibling.querySelector(`label[label_key="${selected_color_image.img_src}"]`);
        selected_image_label.classList.remove("border")
        selected_image_label.classList.add("ring-offset-2", "ring-2", "ring-[#FF375F]")
        setProductImage_url(selected_color_image.img_src)
    }

    const handleColorChangeByImage = (event, image_url) => {
        const element = event.target;
        const allLabels = element.closest('ul').querySelectorAll('.ring-2');
        ...
        parentNode.classList.remove("border")
        parentNode.classList.add("ring-offset-2", "ring-2", "ring-[#FF375F]")
                            ...
                            <div onClick={(e) => e.stopPropagation()} className="relative w-0 aspect-[1/2] max-2md:max-h-[95%] md:aspect-[5/3] bg-white transition-all duration-500 overflow-y-auto overflow-x-hidden rounded-xl p-1.5 sm:p-2">
                                <div className="relative w-full h-full gap-2 flex max-md:flex-col justify-between">
                                    ...
                                    <div className="md:h-full flex-1 md:overflow-y-auto">
                                        ...
                                        <div className="h-11 w-full flex items-center font-semibold font-sans">
                                            ...
                                            <ul className="h-2-scrollbar flex items-center h-full gap-3 sm:gap-5 text-sm overflow-x-auto px-2.5">
                                                {uniqueColors.map((product_color, color_index) => (
                                                    <li key={color_index} className="h-6 sm:h-7 aspect-square">
                                                        ...
                                                        <label htmlFor={`color-input-check-${productGridId + product_index}-${product_color.name}`} className={`block rounded-full shadow-md h-full w-full peer-checked:ring-offset-2 peer-checked:ring-1 sm:peer-checked:ring-2 ring-[#FF375F]  cursor-pointer`} style={{ "backgroundColor": `${product_color.color}`, "background": `${product_color.color}` }} onClick={handleColorChange}></label>
                                                    </li>
                                                ))}
                                            </ul>
                                        </div>
                                        {/* Color Choice by Image */}
                                        <div className="h-12 md:h-16 w-full flex items-center font-semibold font-sans pl-[52px] sm:pl-[60px]">
                                            <ul className="h-2-scrollbar flex items-center h-full gap-2.5 sm:gap-3 text-sm overflow-auto pr-2 pl-1">
                                                {product_details.colors.map((product_color, color_index) => (
                                                    <li key={color_index} className="h-5/6 aspect-square">
                                                        <label label_key={product_color.img_src} htmlFor={`color-input-check-${productGridId + product_index}-${product_color.name}`} className={`block rounded-md h-full w-full cursor-pointer overflow-hidden border border-gray-500`}>
                                                            <Image
                                        ...
                                        <div className="flex items-center gap-6 mb-4 w-full h-8 sm:h-9 mt-5">
                                            <div className="w-40 sm:w-48 h-full box-content bg-transparent text-blue-600 grid grid-cols-3 border border-gray-400 rounded-md overflow-hidden *:flex *:items-center *:justify-center">
                                            ...
                                            <div className="h-full w-full flex gap-2 font-mono text-sm sm:text-base *:w-auto *:h-full *:px-2">
                                        ...
                                        <div className={`${roboto_slab.className} mt-4 font-serif`}>
                                            <p className="text-sm text-black mt-1 font-sans">
                                                <strong>SKU:</strong> black_102288912<br /> <strong> Category:</strong> Bra and Panty set
                                            </p>
                                        </div>
                                        <div className={`text-sm font-sans mt-2 mb-6`}>
                                            <ul className="list-disc pl-7 font-lighter">
                                                {point_description.map((description, index) => (
